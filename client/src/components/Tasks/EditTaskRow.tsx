...
const EditTaskRow = ({ task, onUpdate, onCancel }: EditTaskRowProps) => {
  ...
  const isUpdateDisabled = !title || !dueDate || !status || !category;
  ...
  return (
    <>
      <div className="grid grid-cols-[40px_30px_24px_1fr_150px_150px_150px_40px] items-center px-4 py-3 border-b bg-[#F1F1F1] text-sm relative">
        ...
        <div className="flex flex-col">
          ...
          <div className="flex gap-2 mt-2">
            ...
            >
              SAVE
              <img src={unionIcon} alt="Save" className="mix-blend-brighten" />
            </Button>
            <Button variant="outline" onClick={onCancel} className="border-none">
              CANCEL
            </Button>
          </div>
        </div>
        ...
        {/* Due Date */}
        <div className="relative flex flex-col items-start gap-1" ref={calendarRef}>
          <img
            src={addDateIcon}
            alt="Add Due Date"
          ...
          />
          {dueDate && (
            <span className="text-sm text-gray-700">{dueDate.toLocaleDateString()}</span>
          )}
          {showCalendar && (
        ...
        <div className="flex items-center gap-2 relative">
          ...
          <button
            ...
            onClick={() => {
              ...
              if (rect) {
                setStatusPosition({ top: rect.bottom + window.scrollY + 5, left: rect.left });
              }
              setStatusModalOpen(true);
            }}
          >
            <img src={plusIcon} alt="Change Status" />
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 relative">
          ...
          <button
            ...
            onClick={() => {
              ...
              if (rect) {
                setCategoryPosition({ top: rect.bottom + window.scrollY + 5, left: rect.left });
              }
              setCategoryModalOpen(true);
            }}
          >
            <img src={plusIcon} alt="Change Category" />
          </button>
        </div>
      </div>
      ...
      {statusModalOpen && statusPosition && (
        <Modal onClose={() => setStatusModalOpen(false)} position={statusPosition}>
      ...
      {categoryModalOpen && categoryPosition && (
        <Modal onClose={() => setCategoryModalOpen(false)} position={categoryPosition}>
          <div className="flex flex-col gap-2 py-2">
            {["WORK", "PERSONAL"].map((c) => (
              <Button
                className="border-none text-start ml-2"
